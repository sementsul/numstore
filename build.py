...
def render_calc():
    ...
    body = (
        ...
        '(процедура MNP) — тогда за него берут цену по категории красоты (диапазоны — в расчёте выше). Пока номер '
        'у оператора «Безлимит», вы за сам номер не платите.</p>'
        '<p><b>Регион на цену не влияет.</b> Стоимость определяется красотой номера (категорией), а не кодом или '
        'регионом.</p>'
        '<p class="calc-disc">Калькулятор считает расходы на тариф за срок (номер бесплатный) и показывает '
        'ориентировочную цену переноса по категории. Точные условия — у оператора при оформлении: '
