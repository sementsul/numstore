...
LANDINGS = [
    ...
    {"slug": "v-podarok", "h1": "Номер в подарок",
     ...
     "faq": [
         ("Как подарить красивый номер?", "Подберите номер с личными цифрами получателя (дата, год), забронируйте, а оформление проведёт оператор. SIM или eSIM можно передать имениннику."),
         ("Можно ли выбрать номер с датой рождения?", "Да, задайте день/месяц/год маской на нужных позициях — в каталоге останутся подходящие варианты."),
     ]},
    {"slug": "legko-zapomnit", "h1": "Номера, которые легко запомнить",
     ...
     "faq": [
         ("Какие номера запоминаются лучше всего?", "С повторами одинаковых цифр, парами (ABAB) и круглым окончанием на нули — их мозг схватывает как единый образ, а не как 10 разных цифр."),
         ("Как найти лёгкий для памяти номер?", "Задайте маской привычные цифры (например, повтор в конце) — каталог покажет подходящие комбинации."),
     ]},
    {"slug": "na-datu-rozhdeniya", "h1": "Номер с датой рождения",
     ...
     "faq": [
         ("Как подобрать номер со своей датой рождения?", "Впишите день, месяц или год маской на нужные позиции, остальные оставьте любыми — система покажет номера с этими цифрами."),
