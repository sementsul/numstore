...
- **UC-64:** ВСЕ страницы (app.js) грузят номера из локального `catalog.json`, а не из живого API — `fetchNumbers` → catalog.json (маска/фильтры на клиенте: `maskMatch`/`matchesClient`; категории `catSlugsForPage`), `fetchNumbersLive` — фолбэк. Убран `loadFilters` (лишний API). На загрузке 0 обращений к Безлимиту; API только при брони (живой чек наличия + резервация). catalog.json получил поле `i` (tariff.id). 🔶 локальная база = топ-250/категория → узкий маска-поиск ограничен этим срезом.
