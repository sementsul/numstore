...
- `collect_catalog.py` — тянет до 250 номеров/категория из API → `data/catalog.json` (поля n/t/p + `c` — тарифный тир top/mid/base, классифицируется относительно категории, само-подстраивается). Запускать в CI (у контейнера IP к API в бане, у GH Actions — проходит).
- `.github/workflows/catalog.yml` — крон каждый час: собрать → коммит в numstore (под DEPLOY_TOKEN) → публикация в magzgold. ~1250 номеров, ~73 КБ.
- `premium-demo.html` — ПРЕМИУМ-ЛЕНДИНГ (⏳ превью, будущая главная): сериф-герой «one of one», грузит `catalog.json` (мгновенно, случайные 9 бриллиантовых с классом top (крон классифицирует)), фолбэк на живой API; кнопка «Другие номера / Все бриллиантовые» → `/kategoriya/brilliant/` (поиск/каталог с фильтром БРИЛЛИАНТОВЫХ, не все категории). Бронь = как на сайте: окно с 4 шагами → POST-резервация → ссылка с uuid (флоу 1:1 из app.js); занят → модалка «номер купили».
