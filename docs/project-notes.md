...
- `.github/workflows/catalog.yml` — крон каждые 4ч: собрать → коммит в numstore (под DEPLOY_TOKEN) → публикация в magzgold. ~1250 номеров, ~73 КБ.
- `premium-demo.html` — ПРЕМИУМ-ЛЕНДИНГ (⏳ превью, будущая главная): сериф-герой «one of one», грузит `catalog.json` (мгновенно, случайные 9 бриллиантовых с тарифом ≥4000 ₽/мес — топ), фолбэк на живой API; кнопка «Другие номера / Все бриллиантовые» → `/kategoriya/brilliant/` (поиск/каталог с фильтром БРИЛЛИАНТОВЫХ, не все категории). Бронь = как на сайте: окно с 4 шагами → POST-резервация → ссылка с uuid (флоу 1:1 из app.js); занят → модалка «номер купили».
