...
- `premium-demo.html` — ПРЕМИУМ-ЛЕНДИНГ (⏳ превью, будущая главная): сериф-герой «one of one», грузит `catalog.json` (мгновенно, случайные 9 бриллиантовых), фолбэк на живой API; кнопка «Другие номера» → `/start/`. Бронь = как на сайте: окно с 4 шагами → POST-резервация → ссылка с uuid (флоу 1:1 из app.js); занят → модалка «номер купили».
