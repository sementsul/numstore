...
## Watchdog (свежесть данных)
`.github/workflows/watchdog.yml` — ежедневно тянет `data/price_history.json` из raw и проверяет возраст свежайшей точки; >48ч (два пропущенных дня) → воркфлоу ПАДАЕТ → GitHub шлёт владельцу письмо. Ловит: заснувший крон (истёк DEPLOY_TOKEN), поломку сбора цен, падение публикации. Аналог сторожа в ratescout (UC-108).
